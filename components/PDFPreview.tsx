...
import { ArrowLeft, Download, FileText, Sparkles, Loader2, User, Mail, Phone, Briefcase, FileUser, Award } from 'lucide-react';
          ...
          <CardContent>
            <div className="bg-gradient-to-br from-slate-50 to-blue-50 p-8 rounded-xl min-h-[700px] space-y-8">
              ...
              <div className="text-center border-b-4 border-blue-500 pb-6">
                ...
                <p className="text-sm text-slate-500 mt-2">Generated on {new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
              </div>
      ...
      <div className="lg:w-96">
        <Card className="shadow-2xl border-0 bg-white/90 backdrop-blur-sm sticky top-8 space-y-6">
          ...
          <CardContent className="space-y-6">
              ...
              <div className="text-sm text-blue-700 space-y-2">
                ...
                {userData.position && (
                  <div className="flex gap-2 items-start">
                    ...
                    <div><span className="font-medium">Position:</span> {userData.position}</div>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-5 rounded-xl border border-purple-200 space-y-3">
              ...
              <div className="text-sm space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={aiEnhancements.suggestedSkills?.length ? "default" : "secondary"} className="text-xs">
                    {aiEnhancements.suggestedSkills?.length ? `✓ ${aiEnhancements.suggestedSkills.length} skills` : "Pending"}
                  </Badge>
                </div>
                {isLoadingAI && (
            ...
            {/* Buttons */}
            <Button onClick={handleBack} variant="outline" className="w-full border-slate-300 text-slate-700 hover:bg-slate-50 py-3 transition-all hover:scale-[1.02]" size="lg">
              <ArrowLeft className="w-5 h-5 mr-2" />
              Back to Form
            </Button>

            <Button onClick={handleDownload} disabled={isDownloading} className="w-full bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white py-3 transition-all hover:scale-[1.02] shadow-lg" size="lg">
